...
def display_train_squat_page():
    ...
    st.subheader("Pilih Data untuk Pelatihan Squat")
    uploaded_file = st.file_uploader("Pilih File JSON untuk Squat", type=["json"])
    folder_path = st.text_input("Masukkan Path Folder JSON untuk Squat (opsional)", "")
    ...
    if train_button and st.session_state.data:
        ...
        joblib.dump(final_model, 'gesture_classifier_squat_cv.pkl')
        output_container.success("Model akhir disimpan sebagai 'gesture_classifier_squat_cv.pkl'")
        st.session_state.training_status = False