...
def draw_info(image, prediction, count):
    # Draw the predicted gesture and squat count on the image
    cv2.putText(
        image,
        f"Pose: {prediction}",
        (10, 30),
...
def display_squat_counter_page():
    # Check if classifier model exists
    if not os.path.exists(CLASSIFIER_PATH):
        st.error("Model 'gesture_classifier_squat_cv.pkl' tidak ditemukan. Silakan latih model untuk squat terlebih dahulu.")
        return
    ...
    while True:
        ...
        draw_info(frame, predicted_gesture, st.session_state.squat_count)
